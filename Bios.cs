	...
	class Bios
	{
		...
		public Bios(uint startAddress, InterconnectTerminal systemInterconnect)
		{
			...
			m_biosData = new int[] {
										...
										(int)UnitCodes.Load			|	(int)LoadOperations.LoadFromLiteralLocation		|	0 << 8	|	0,	(int)Program.SSDFIFOAddress,			// Load number of blocks in program into register 0
										
										// Init program read
										(int)UnitCodes.ALU			|	(int)ALUOperations.SetLiteral					|	1 << 8	|	0,	1,										// Set initial block
										(int)UnitCodes.Store		|	(int)StoreOperations.StoreToLiteralLocation		|	0 << 8	|	1,	(int)Program.SSDSeekAddress,			// Set storage
										...
										(int)UnitCodes.Load			|	(int)LoadOperations.LoadFromLiteralLocation		|	5 << 8	|	0,	(int)Program.SSDFIFOAddress,			// Load value into r5
										(int)UnitCodes.Store		|	(int)StoreOperations.StoreToRegisterLocation	|	3 << 8	|	5,	(int)Program.RAMStartAddress,			// Store value from r5 into RAM
										...
										(int)UnitCodes.Branch		|	(int)BranchOperations.JumpLess					|	2 << 8	|	4,	(int)Program.biosStartAddress + 36,		// Loop through block
										...
										(int)UnitCodes.Branch		|	(int)BranchOperations.JumpLess					|	1 << 8	|	0,	(int)Program.biosStartAddress + 36,		// Loop through block

										(int)UnitCodes.Branch		|	(int)BranchOperations.Jump						|	0 << 8	|	0,	(int)Program.RAMStartAddress,			// Jump to start of program just loaded
